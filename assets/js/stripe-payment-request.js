/* global wc_monilypay_payment_request_params, Stripe */
jQuery( function( $ ) {
	'use strict';
	...
	var wc_monilypay_payment_request = {
		...
		getAjaxURL: function( endpoint ) {
			return wc_monilypay_payment_request_params.ajax_url
				.toString()
		...
		getCartDetails: function() {
			var data = {
				security: wc_monilypay_payment_request_params.nonce.payment
			};

			$.ajax( {
				type:    'POST',
				data:    data,
				url:     wc_monilypay_payment_request.getAjaxURL( 'get_cart_details' ),
				success: function( response ) {
					wc_monilypay_payment_request.startPaymentRequest( response );
				}
			} );
		},
		...
		processSource: function( source, paymentRequestType ) {
			var data = wc_monilypay_payment_request.getOrderData( source, paymentRequestType );

			return $.ajax( {
				type:    'POST',
				data:    data,
				dataType: 'json',
				url:     wc_monilypay_payment_request.getAjaxURL( 'create_order' )
			} );
		},
		...
		getOrderData: function( evt, paymentRequestType ) {
			...
			var data     = {
				_wpnonce:                  wc_monilypay_payment_request_params.nonce.checkout,
		...
		displayErrorMessage: function( message ) {
			...
			if ( wc_monilypay_payment_request_params.is_product_page ) {
		...
		abortPayment: function( payment, message ) {
			...
			wc_monilypay_payment_request.displayErrorMessage( message );
		},
		...
		completePayment: function( payment, url ) {
			wc_monilypay_payment_request.block();
		...
		updateShippingOptions: function( details, address ) {
			var data = {
				security:  wc_monilypay_payment_request_params.nonce.shipping,
				...
				payment_request_type: paymentRequestType,
				is_product_page: wc_monilypay_payment_request_params.is_product_page,
			};

			return $.ajax( {
				type:    'POST',
				data:    data,
				url:     wc_monilypay_payment_request.getAjaxURL( 'get_shipping_options' )
			} );
		},
		...
		updateShippingDetails: function( details, shippingOption ) {
			var data = {
				security: wc_monilypay_payment_request_params.nonce.update_shipping,
			...
			return $.ajax( {
				type: 'POST',
				data: data,
				url:  wc_monilypay_payment_request.getAjaxURL( 'update_shipping_method' )
			} );
		},
		...
		addToCart: function() {
			...
			var data = {
				security: wc_monilypay_payment_request_params.nonce.add_to_cart,
				product_id: product_id,
				qty: $( '.quantity .qty' ).val(),
				attributes: $( '.variations_form' ).length ? wc_monilypay_payment_request.getAttributes().data : []
			};
			...
			return $.ajax( {
				type: 'POST',
				data: data,
				url:  wc_monilypay_payment_request.getAjaxURL( 'add_to_cart' )
			} );
		},

		clearCart: function() {
			var data = {
					'security': wc_monilypay_payment_request_params.nonce.clear_cart
				};

			return $.ajax( {
				type:    'POST',
				data:    data,
				url:     wc_monilypay_payment_request.getAjaxURL( 'clear_cart' ),
				success: function( response ) {}
			} );
		},
		...
		getRequestOptionsFromLocal: function() {
			return {
				total: wc_monilypay_payment_request_params.product.total,
				currency: wc_monilypay_payment_request_params.checkout.currency_code,
				country: wc_monilypay_payment_request_params.checkout.country_code,
				requestPayerName: true,
				requestPayerEmail: true,
				requestPayerPhone: wc_monilypay_payment_request_params.checkout.needs_payer_phone,
				requestShipping: wc_monilypay_payment_request_params.product.requestShipping,
				displayItems: wc_monilypay_payment_request_params.product.displayItems
			};
		},
		...
		startPaymentRequest: function( cart ) {
			...
			if ( wc_monilypay_payment_request_params.is_product_page ) {
				options = wc_monilypay_payment_request.getRequestOptionsFromLocal();
			...
			try {
				...
				var elements = stripe.elements( { locale: wc_monilypay_payment_request_params.button.locale } );
				var prButton = wc_monilypay_payment_request.createPaymentRequestButton( elements, paymentRequest );
				...
				paymentRequest.canMakePayment().then( function( result ) {
					...
					wc_monilypay_payment_request.attachPaymentRequestButtonEventListeners( prButton, paymentRequest );
					wc_monilypay_payment_request.showPaymentRequestButton( prButton );
				} );
				...
				paymentRequest.on( 'shippingaddresschange', function( evt ) {
					$.when( wc_monilypay_payment_request.updateShippingOptions( paymentDetails, evt.shippingAddress ) ).then( function( response ) {
						evt.updateWith( { status: response.result, shippingOptions: response.shipping_options, total: response.total, displayItems: response.displayItems } );
					} );
				} );

				paymentRequest.on( 'shippingoptionchange', function( evt ) {
					$.when( wc_monilypay_payment_request.updateShippingDetails( paymentDetails, evt.shippingOption ) ).then( function( response ) {
				...
				paymentRequest.on( 'source', function( evt ) {
					...
					if ( 'no' === wc_monilypay_payment_request_params.monilypay.allow_prepaid_card && 'prepaid' === evt.source.card.funding ) {
						...
						$.when( wc_monilypay_payment_request.processSource( evt, paymentRequestType ) ).then( function( response ) {
							if ( 'success' === response.result ) {
								wc_monilypay_payment_request.completePayment( evt, response.redirect );
							} else {
								wc_monilypay_payment_request.abortPayment( evt, response.messages );
							}
						} );
					}
				} );
		...
		getSelectedProductData: function() {
			...
			var data = {
				security: wc_monilypay_payment_request_params.nonce.get_selected_product_data,
				product_id: product_id,
				qty: $( '.quantity .qty' ).val(),
				attributes: $( '.variations_form' ).length ? wc_monilypay_payment_request.getAttributes().data : [],
			...
			return $.ajax( {
				type: 'POST',
				data: data,
				url:  wc_monilypay_payment_request.getAjaxURL( 'get_selected_product_data' )
			} );
		},
		...
		createPaymentRequestButton: function( elements, paymentRequest ) {
			var button;
			if ( wc_monilypay_payment_request_params.button.is_custom ) {
				button = $( wc_monilypay_payment_request_params.button.css_selector );
				if ( button.length ) {
			...
			if ( wc_monilypay_payment_request_params.button.is_branded ) {
				if ( wc_monilypay_payment_request.shouldUseGooglePayBrand() ) {
					button = wc_monilypay_payment_request.createGooglePayButton();
					...
					wc_monilypay_payment_request_params.button.type = 'long' === wc_monilypay_payment_request_params.button.branded_type ? 'buy' : 'default';
				}
			}
			...
			return elements.create( 'paymentRequestButton', {
				...
				style: {
					paymentRequestButton: {
						type: wc_monilypay_payment_request_params.button.type,
						theme: wc_monilypay_payment_request_params.button.theme,
						height: wc_monilypay_payment_request_params.button.height + 'px',
					},
				},
			} );
		},
		...
		createGooglePayButton: function () {
			...
			var theme  = wc_monilypay_payment_request_params.button.theme;
			var type   = wc_monilypay_payment_request_params.button.branded_type;
			var locale = wc_monilypay_payment_request_params.button.locale;
			var height = wc_monilypay_payment_request_params.button.height;
		...
		attachPaymentRequestButtonEventListeners: function( prButton, paymentRequest ) {
			...
			if ( wc_monilypay_payment_request_params.is_product_page ) {
				wc_monilypay_payment_request.attachProductPageEventListeners( prButton, paymentRequest );
			} else {
				wc_monilypay_payment_request.attachCartPageEventListeners( prButton, paymentRequest );
			}
		},
		...
		attachProductPageEventListeners: function( prButton, paymentRequest ) {
			...
			prButton.on( 'click', function ( evt ) {
				...
				if ( wc_monilypay_payment_request_params.login_confirmation ) {
					evt.preventDefault();
			...
			$( document.body ).on( 'wc_monilypay_unblock_payment_request_button wc_stripe_enable_payment_request_button', function () {
				wc_monilypay_payment_request.unblockPaymentRequestButton();
			} );

			$( document.body ).on( 'wc_monilypay_block_payment_request_button', function () {
				wc_monilypay_payment_request.blockPaymentRequestButton( 'wc_request_button_is_blocked' );
			} );

			$( document.body ).on( 'wc_monilypay_disable_payment_request_button', function () {
				wc_monilypay_payment_request.blockPaymentRequestButton( 'wc_request_button_is_disabled' );
			} );

			$( document.body ).on( 'woocommerce_variation_has_changed', function () {
				$( document.body ).trigger( 'wc_monilypay_block_payment_request_button' );

				$.when( wc_monilypay_payment_request.getSelectedProductData() ).then( function ( response ) {
					$.when(
						...
						$( document.body ).trigger( 'wc_monilypay_unblock_payment_request_button' );
					} );
				});
			} );
			...
			$( '.quantity' ).on( 'input', '.qty', wc_monilypay_payment_request.debounce( 250, function() {
				$( document.body ).trigger( 'wc_monilypay_block_payment_request_button' );
				paymentRequestError = [];
			...
			if ( $('.variations_form').length ) {
				$( '.variations_form' ).on( 'found_variation.wc-variation-form', function ( evt, variation ) {
					if ( variation.is_in_stock ) {
						wc_monilypay_payment_request.unhidePaymentRequestButton();
					} else {
						wc_monilypay_payment_request.hidePaymentRequestButton();
					}
				} );
			}
		},
		...
		attachCartPageEventListeners: function ( prButton, paymentRequest ) {
			prButton.on( 'click', function ( evt ) {
				...
				if (
					wc_monilypay_payment_request.isCustomPaymentRequestButton(
						prButton
					) ||
					wc_monilypay_payment_request.isBrandedPaymentRequestButton(
						prButton
					)
				) {
					evt.preventDefault();
		...
		showPaymentRequestButton: function( prButton ) {
			if ( wc_monilypay_payment_request.isCustomPaymentRequestButton( prButton ) ) {
				prButton.addClass( 'is-active' );
				$( '#wc-stripe-payment-request-wrapper, #wc-stripe-payment-request-button-separator' ).show();
			} else if ( wc_monilypay_payment_request.isBrandedPaymentRequestButton( prButton ) ) {
				$( '#wc-stripe-payment-request-wrapper, #wc-stripe-payment-request-button-separator' ).show();
		...
		init: function() {
			if ( wc_monilypay_payment_request_params.is_product_page ) {
				wc_monilypay_payment_request.startPaymentRequest( '' );
			} else {
				wc_monilypay_payment_request.getCartDetails();
			}

		},
	};

	wc_monilypay_payment_request.init();
	...
	function displayLoginConfirmation( paymentRequestType ) {
		...
		var message = wc_monilypay_payment_request_params.login_confirmation.message;
		...
		if ( confirm( message ) ) {
			...
			window.location.href = wc_monilypay_payment_request_params.login_confirmation.redirect_url;
		}
	}
} );
